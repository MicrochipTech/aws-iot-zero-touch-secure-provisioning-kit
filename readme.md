...
1. Clone or Download the AWS IOT Zero-Touch Secure Provisioning kit software.
   ...
   **Note**- *CryptoAuthLib is a submodule and is not automatically included
   and will need to be cloned recursively or downloaded separately from the kit
   software. If downloaded separately, the files will need to be placed in
   firmware\SAMG55\AWS_IoT_Zero_Touch_SAMG55\src\cryptoauthlib folder*
...
6. Maybe install **Visual C++ 2017 Build Tools**.
   Microsoft Visual C++ Build Tools 14.0 may be required for the hidapi python
   package. This is a big download/install and it is not needed if the previous
...
1. Once AWS has been configured successfully, Run ```aws configure``` from the
   command line. (part of the AWS CLI download)
...
3. Plug **CryptoAuth Xplained Pro** into **EXT4** on the SAMG55 Xplained Pro.
   Please note, depending on when you purchased your kit, your kit may have
   come with CryptoAuth Xplained Pro **Rev A** boards or **Rev B** boards.
   Rev B boards have an ATECC608A device attached and do not come
   pre-configured.  Extra steps need to be followed to initialize the
   ATECC CryptoAuthentication device on the board.  Begin the initialization
   process by **running the firmware without the WINC1500 Xplained Pro board
   attached**.  The firmware will automatically guide you through this process
   with instructions from EDBG serial port output messages.
...
3. Once the firmware has been successfully downloaded open a serial terminal
   and press the reset button on the SAMG55. There should be instructions on
   how to proceed.
...
**Note** - If the AWS CloudFormation template was used these two steps can be
skipped.
...
1. Run ```python ca_create_root.py``` to create a root CA.
2. Run ```python ca_create_signer_csr.py``` to create a CSR for a signer.
3. Run ```python ca_create_signer.py``` to sign the signer CSR with the root CA.
4. Run ```python aws_register_signer.py``` to register the signer with AWS IoT.

### Provision the ATECCx08A on the kit

1. Run ```python kit_set_wifi.py --ssid wifi-name --password wifi-password```
...
2. Run ```python kit_provision.py``` to provision the ATECCx08A on the board
   for AWS IoT. After this command, the board will automatically attempt to
   connect to AWS IoT.

### Interact with the Board via AWS

1. Run ```python aws_interact_gui.py``` to interact with the board and toggle
